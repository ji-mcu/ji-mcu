...
### my-ubuntu使用方法
  ...
  8. 在vendor里最重要的文件有core_SFR.h、soc_common_.h、soc_event.h,
      分别存放了寄存器和数据地址、外设驱动库头文件、中断与异常函数驱动，
      所有外设驱动的文件风格为IC_XXX,什么外设就命名为什么，比如IC_SPI就是SPI的外设驱动。
     
  9. 在vendor的终端下，编译使用make all生成编译文件， make clean清除编译文件，make kgdb用于停止gdb下载打开的端口，make code 生成反汇编代码并存放在build文件夹，
