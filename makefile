...
run:
	...
	source env.sh && cd $(runner_build_dir) && python bin/plp_mkflash --flash-boot-binary=$(join $(join $(app_build_dir),$(APPNAME)),/build/$(PULP_APP_NAME))  --stimuli=flash_stim.slm --flash-type=spi --qpi --archi=pulpissimo --raw flash.bin --verbose
