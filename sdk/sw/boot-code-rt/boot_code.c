...
// #define DSPI  1
// #define QSPI  0
#define BOOT_STACK_SIZE  256
#define MAX_NB_AREA 4
...
typedef struct {
  unsigned char flashBuffer[260608]; //0x1FC00
  unsigned int udma_buffer[16];
  int spi_flash_id;
  int step;
...
void flash_RDSR(boot_code_t *data)
{
    unsigned int *buffer = data->udma_buffer;
    *(volatile int *)&buffer[0] = 0x2;
    *(volatile int *)&buffer[1] = SPI_CMD_SOT(0);
    *(volatile int *)&buffer[2] = SPI_CMD_SEND_CMD(0x35,8,0);//set command WriteEnable
    *(volatile int *)&buffer[3] = SPI_CMD_RX_DATA(8, 0, 0, SPI_CMD_BYTE_ALIGN_ENA);
    *(volatile int *)&buffer[4] = SPI_CMD_EOT(1);

    plp_udma_enqueue(UDMA_SPIM_TX_ADDR(0), (unsigned int)data->udma_buffer, 5*4, UDMA_CHANNEL_CFG_EN | UDMA_CHANNEL_CFG_SIZE_32);
    plp_udma_enqueue(UDMA_SPIM_RX_ADDR(0), (unsigned int)&data->status_register, 1, UDMA_CHANNEL_CFG_EN | UDMA_CHANNEL_CFG_SIZE_32);
    wait_soc_event();
}

// void flash_write_cmd(boot_code_t *data)
// {
//     unsigned int *buffer = data->udma_buffer;
//     *(volatile int *)&buffer[0] = 0x0;
//     *(volatile int *)&buffer[1] = SPI_CMD_SOT(0);
//     *(volatile int *)&buffer[2] = SPI_CMD_SEND_CMD(0x06,8,0);//set command WriteEnable
//     *(volatile int *)&buffer[3] = SPI_CMD_EOT(1);

//     plp_udma_enqueue(UDMA_SPIM_TX_ADDR(0), (unsigned int)data->udma_buffer, 4*4, UDMA_CHANNEL_CFG_EN | UDMA_CHANNEL_CFG_SIZE_32);
//     wait_soc_event();
// }

// void flash_write_status(boot_code_t *data)
// {
//     unsigned int *buffer = data->udma_buffer;
//     *(volatile int *)&buffer[0] = 0x0;
//     *(volatile int *)&buffer[1] = SPI_CMD_SOT(0);
//     *(volatile int *)&buffer[2] = SPI_CMD_SEND_CMD(0x01,8,0);//set command WRSR
//     *(volatile int *)&buffer[3] = SPI_CMD_TX_DATA(16, 0, 0);
//     *(volatile int *)&buffer[4] = 0x0200; //set SREG_QE
//     *(volatile int *)&buffer[5] = SPI_CMD_EOT(1);

//     plp_udma_enqueue(UDMA_SPIM_TX_ADDR(0), (unsigned int)data->udma_buffer, 6*4, UDMA_CHANNEL_CFG_EN | UDMA_CHANNEL_CFG_SIZE_32);
//     wait_soc_event();
// }  
...
static void flash_read(boot_code_t *data, unsigned int flashAddr, unsigned int l2Addr, unsigned int size)
{
  if (!data->hyperflash) {
    ...
    *(volatile int *)&buffer[0] = SPI_CMD_CFG       (2,0,0);
    *(volatile int *)&buffer[1] = SPI_CMD_SOT       (0);     
    if(qpi_mode){
        *(volatile int *)&buffer[2] = SPI_CMD_SEND_CMD  (0x6b, 8, 0);
        *(volatile int *)&buffer[3] = SPI_CMD_SEND_ADDR (32, 0);
        *(volatile int *)&buffer[4] = SPI_CMD_SEND_ADDR_VALUE(flashAddr<<8);
        *(volatile int *)&buffer[5] = SPI_CMD_RX_DATA(size*8, 0, 1, SPI_CMD_BYTE_ALIGN_ENA);
    }
    else{
        *(volatile int *)&buffer[2] = SPI_CMD_SEND_CMD  (0x3b, 8, 0);
        *(volatile int *)&buffer[3] = SPI_CMD_SEND_ADDR (32, 0);
        *(volatile int *)&buffer[4] = SPI_CMD_SEND_ADDR_VALUE(flashAddr<<8);
        *(volatile int *)&buffer[5] = SPI_CMD_RX_DATA(size*8, 1, 0, SPI_CMD_BYTE_ALIGN_ENA);
        // *(volatile int *)&buffer[2] = SPI_CMD_SEND_CMD  (0x03, 8, 0);
        // *(volatile int *)&buffer[3] = SPI_CMD_SEND_ADDR (24, 0);
        // *(volatile int *)&buffer[4] = SPI_CMD_SEND_ADDR_VALUE(flashAddr<<8);
        // *(volatile int *)&buffer[5] = SPI_CMD_RX_DATA(size*8, 0, 0, SPI_CMD_BYTE_ALIGN_ENA);
      }
    // *(volatile int *)&buffer[4] = SPI_CMD_DUMMY     (15);
    // *(volatile int *)&buffer[5] = SPI_CMD_RX_DATA(size, SPI_CMD_4_WORD_PER_TRANSF, 8, SPI_CMD_QPI_DIS, SPI_CMD_MSB_FIRST);
    *(volatile int *)&buffer[6] = SPI_CMD_EOT       (1);

    // unsigned int channel_id = ARCHI_UDMA_SPIM_ID(0) * 2;
...
static void getMemAreas(boot_code_t *data)
{
  while(1){
    ...
    if(((data->header.bootaddr&0xFFF00000) == 0x1C000000) && ((data->header.entry&0xFFF00000) == 0x1C000000))
      break;
  }
...
static __attribute__((noreturn)) void loadBinaryAndStart(boot_code_t *data)
{
  ...
  flash_RDSR(data);
  data->qpi = (data->status_register>>1)&&0x1;
...
static void bootFromRom(int hyperflash, int qpi)
{
  ...
  // #if DSPI
  //   flash_write_cmd(data); //write enable
  // #endif

  // #if QSPI
  //   flash_write_cmd(data); //write enable
  //   flash_write_status(data); //write status QPI Mode
  // #endif
  // getMemAreas(data);
  ...
  loadBinaryAndStart_newStack(data);
